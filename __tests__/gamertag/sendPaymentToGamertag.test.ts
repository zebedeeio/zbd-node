...
import { SendGamertagPaymentOptionsType, isSendGamertagPaymentDataResponseType } from '../../src/types';
...
describe('Send Payment to Gamertag', () => {
  ...
  describe('sendPaymentToGamertag error scenarios', () => {
    ...
    it('should throw an error given a non-existent gamertag', async () => {
      ...
      await expect(ZBD.sendGamertagPayment(errorRequestBody)).rejects.toMatchObject({
