...
import {
  ...
  isGetWithdrawalRequestDataResponseType,
} from "../../src/types";
...
describe("createWithdrawalRequest", () => {
  ...
  it("should successfully create a withdrawal request", async () => {
    const responseData: CreateWithdrawalRequestDataResponseType =
      await ZBD.createWithdrawalRequest(requestBody);

    expect(responseData.success).toBe(true);
    expect(responseData.message).toBe(
      "Successfully created Withdrawal Request."
    );
    ...
    // Data Validation
    expect(
      isCreateWithdrawalRequestDataResponseType(responseData)
    ).toBeTruthy();
  });
  ...
  describe("createWithdrawalRequest error scenarios", () => {
    it("should throw an error given a faulty payload (amount invalid)", async () => {
      ...
      await expect(
        ZBD.createWithdrawalRequest(faultyRequestBody)
      ).rejects.toMatchObject({
        message:
          'The "amount" property (in millisatoshis) must be greater than 1 sat.',
        status: 400,
      });
    });
  });
