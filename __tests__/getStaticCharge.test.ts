...
import { StaticChargeDataResponseType, isStaticChargeDataResponseType } from '../src/types/static-charges'; 
...
describe('getStaticCharge', () => {
  ...
  it('should successfully fetch Static Charge details', async () => {
    ...
    expect(responseData.message).toBe('Successfully retrieved Static Charge data.');
  ...
  it('should return a 404 for a non-existent charge ID', async () => {
    ...
    await expect(ZBD.getCharge(NON_EXISTENT_STATIC_CHARGE_ID)).rejects.toMatchObject({
